...
import { Colors, getPrice, Timings } from '../utils';

export class Pair extends Entity<PairShape> implements Resolvable {

    static ABI: AbiItem[] = require('../../abis/pair.json').abi as AbiItem[];
    static uid: UniqueID = 'Pair';
    ...
    get should_save() :boolean { return this.shape.token0 !== undefined && this.shape.token1 !== undefined; }
    ...
    */
    ...
    async tvl({safe, router}: {safe: SafeWeb3, router: string}): Promise<number> {
        ...
        try{
            ...
            const token0 = new safe.w3.eth.Contract(Token.ABI, this.shape.token0);
            const token1 = new safe.w3.eth.Contract(Token.ABI, this.shape.token1);
    ...
    async token0(db: Database): Promise<Token | undefined> {
        if(!this.shape.token0) return undefined;
        let t0 = await Token.get(db, `address="${this.shape.token0}"`);
    ...
    async token1(db: Database): Promise<Token | undefined> {
        if(!this.shape.token1) return undefined;
        let t1 = await Token.get(db, `address="${this.shape.token1}"`);
    ...
    async filter(safe: SafeWeb3): Promise<Pair | null> {
        ...
        if(entity_name === "Pair") return this;
        else return null;
    }
